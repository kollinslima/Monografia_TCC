...
\par Como pode ser observado, o simulador e o Arduino se comportam de maneira semelhantes tanto na geração de frequência quanto nas características do \textit{duty cicle}, apresentando um erro relativo alto quando se trabalha com altas frequências. 
...
\par O aparelho utilizado em todos os testes foi um \textit{smartphone} ASUS ZenFone 2, com 4 núcleos de processamento (2,33GHz), 4GB de memória principal e 32GB de armazenamento, executando o Android 5.0.
...
\begin{figure}[h!]
	...
	\caption{Tamanho do APK (MB) de cada aplicativo.} 
	Fonte: Autor
	\label{fig:comp_tamanho}
\end{figure} 

\par Pode-se observar que o \textit{Arduino Simulator Mini Free} possui um tamanho muito superior aos demais, resultado este um tanto inesperado, uma vez que a versão \textit{Full Pack} deste aplicativo possui um tamanho de 9MB.

\subsubsection{Uso de CPU}

\par O uso de CPU foi medido utilizando um \textit{shell} remoto acessado pelo \textit{Android Debug Bridge} (adb). O comando \textit{top} pode ser utilizado de modo análogo ao \textit{htop} dos sistemas Linux, e mostra diversas informações a respeito dos processos em execução, incluindo o uso de CPU. O gráfico da figura~\ref{fig:comp_cpu} mostra os resultados obtidos das medições, com indicação do desvio médio (para cada processo, foram coletadas 397 amostras).

\par O aplicativo \textit{AndMCU} foi testado em duas condições diferentes, para alta e baixa velocidade de simulação definida pela diretriz \textit{.speed}.

\begin{figure}[h!]
	\centering
	\includegraphics[width=\textwidth]{./Resources/uso_cpu}
	\caption{Consumo médio de CPU (\%) de cada aplicativo.} 
	Fonte: Autor
	\label{fig:comp_cpu}
\end{figure} 

\par É difícil comparar o resultado do \textit{AndMCU} com os demais aplicativos pois, em todos os demais projetos, o código foi o mesmo (em C), a plataforma é a mesma (Arduino) e a frequência de intermitência é a mesma (500Hz), enquanto que para o \textit{AndMCU} está em outro contexto, com código em \textit{assembly}, produzindo uma frequência de intermitência difícil de ser determinada. Apesar disso, o este aplicativo apresentou um consumo de CPU bastante reduzido em comparação aos demais, mesmo quando a velocidade de simulação foi configurada para a máxima possível.

\par Por outro lado, o simulador \textit{BoardMicro - AVR Simulator} apresentou um consumo maior que o dobro da média dos demais, resultado este que provavelmente está relacionado com sua velocidade de simulação, como mostra a seção~\ref{velocidade_simulacao}, enquanto o projeto SOFIA e o \textit{Arduino Simulator Mini Free} apresentaram resultados próximos da média.

\par Importante resaltar que, diretamente relacionado com o uso de CPU, está o consumo de bateria de cada aplicativo. 

\subsubsection{Uso de Memória}

\par O uso de memória por cada aplicativo foi também medido do o uso do \textit{shell} remoto, com o comando \textit{dumpsys meminfo}. Segundo a documentação desta instrução~\cite{Meminfo}, o campo \textit{Proportional Set Size} (PSS), que é a soma da memória privada (pertencente apenas ao aplicativo e liberada ao sistema quando este é encerrado) e da memória compartilhada (total de memória compartilhada dividida pelo número de processos que compartilham a memória), fornece uma boa medida do "peso" deste processo na memória RAM e portanto, este foi o valor utilizado para comparar o uso de memória entre os aplicativos. O gráfico da figura~\ref{fig:comp_ram} mostra esta comparação, com indicação do desvio médio (assim como na CPU, foram realizadas 1000 medições).

\begin{figure}[h!]
	\centering
	\includegraphics[width=\textwidth]{./Resources/uso_ram}
	\caption{Consumo de memória (PSS em MB) de cada aplicativo.} 
	Fonte: Autor
	\label{fig:comp_ram}
\end{figure} 
...
\par Também foi interessante notar, no \textit{AndMCU}, que a configuração de maior velocidade teve um consumo de memória ligeiramente menor que o modo de baixa velocidade. A explicação para isso provavelmente se deve à taxa de atualização da tela nos diferentes modos, como explicado na documentação~\cite{AndMCU}, o modo de alta velocidade diminui a taxa de atualização e o oposto ocorre para o modo de baixa velocidade. Na seção~\ref{profiling} foi mostrado que este parâmetro tem grande impacto no consumo de CPU e de memória no Android.
...
\par A velocidade de simulação aqui apresentada é uma medida da percepção que o usuário quando comparado com o tempo do mundo real.

\par Velocidade de simulação provavelmente é a maior limitação do projeto SOFIA. É possível ter uma ideia clara da velocidade de simulação neste aplicativo uma vez é apresenta a contagem do tempo simulado para o usuário. Este tempo simulado está reduzido em torno de 500x em comparação com o tempo real, já que são contados 2ms a cada segundo. Assim, o usuário percebe, na prática, um sistema com frequência de \textit{clock} efetiva de 32kHz, já que o \textit{hardware} no qual o simulador se baseia é o Arduino Uno rodando à uma frequência de 16MHz.

\par Para os demais simuladores, a velocidade de simulação foi medida alterando o projeto \textit{Blink} para que o tempo de \textit{delay} fosse de 1s. O \textit{Arduino Simulator Mini Free} se mostra bastante fiel ao tempo real, exibindo, de fato, uma intermitência de 1s, fazendo com que o usuário não perceba a diferença deste com um sistema real. Já o simulador \textit{BoardMicro - AVR Simulator} se mostra acelerado, com uma frequência  de intermitência 4x maior que a esperada.

\par Para o projeto \textit{AndMCU}, não foi possível ter uma ideia de sua real velocidade de simulação pois, como já mencionado anteriormente, este aplicativo está em outro contexto do qual não é bem determinado/conhecido como no caso do Arduino. No entanto, é possível ler na documentação do projeto que o sistema possui uma velocidade de simulação adequada para que os resultados possam ser observados pelos estudantes, ou, como o autor coloca em outras palavras, não se deve esperar deste simulador que uma velocidade elevada como o de um emulador de CPU~\cite{AndMCU}.

\subsection{Documentação}

\par O projeto SOFIA conta com uma documentação \textit{on-line} hospedada no Gitbook. Esta documentação pode ser acessada a partir do repositório do Github ou de dentro do próprio aplicativo, nos menus de ajuda e de informações do projeto.

\par A documentação do projeto SOFIA é dividido em 3 partes. A primeira apresenta uma visão geral, apresentando uma breve descrição do que é o projeto, seus objetivos, público alvo, etc. A segunda parte é um manual do usuário e descreve passo-a-passo quais os procedimentos para baixar e instalar o simulador (também a versão modificada da IDE do Arduino) e como usá-lo, apresentando todos os seus recursos. Por fim, a última parte apresenta uma visão mais voltada ao desenvolvimento do projeto, com diagramas, métricas de \textit{software} e comentários a respeito do código desenvolvido. O \textit{link} para acessar a documentação do projeto SOFIA é mostrado na seção~\ref{codigo_documentacao}.

\par O aplicativo \textit{AndMCU} também conta com duas páginas de documentação \textit{on-line} hospedadas no Google Sites e que podem ser acessadas a partir da página de \textit{download} do aplicativo na \textit{Play Store}.

\par Essa documentação apresenta as características e funcionalidades do sistema; o conjunto de instruções implementadas no simulador, bem como algumas diretrizes em \textit{assembly} que podem ser utilizadas; detalham a organização do processador que foi implementado, mostrando seu mapa de memória, esquema de ligação das entradas e saída e uma explicação sobre a CPU e seus registradores; apresenta o \textit{hardware} virtual; entre outras informações. Abaixo é mostrado o endereço eletrônico onde este material pode ser acessado
...
\par Pouca informação está disponível para os projetos \textit{BoardMicro - AVR Simulator} e \textit{Arduino Simulator Mini Free}, além das descrições das páginas de \textit{download} destes aplicativos. O projeto \textit{BoardMicro - AVR Simulator} conta apenas com um breve arquivo \textit{README} em seu repositório no Github, disponível no \textit{link} abaixo:
...
\par Além do código, foi escrita uma documentação referente ao projeto no Gitbook, que pode ser acessada a partir do repositório do projeto ou nos menus "Ajuda" e "Sobre" do aplicativo.

\begin{itemize}
\item \textbf{Documentação SOFIA:} \url{https://project-sofia.gitbook.io/project/}
\end{itemize}