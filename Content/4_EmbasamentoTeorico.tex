...
\par Neste capítulo será explicado o funcionamento e as características de cada módulo presente no microcontrolador do Arduino UNO (ATmega328P) que foram implementados no simulador. Todas as informações foram retiradas da folha de dados do componente~\cite{atmega328p_datasheet}, exceto onde indicado.
...
\par Onde $V_{in}$ é a tensão aplicada à entrada do microcontrolador e ADC é o valor (decimal) convertido. Valores de entrada superiores à $V_{ref}$ terão valores convertidos próximos à 0x3FF (máximo valor para 10-bits).
