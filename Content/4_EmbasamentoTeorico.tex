...
\par Como aponta Jeff Sutherland~\cite{Jeff}, a realidade do desenvolvimento de \textit{software} é diferente do que propõe alguns processos convencionais, como o método cascata, onde o planejamento é inteiramente e minunciosamente detalhado para o desenvolvimento posterior. Alterações nos requisitos, erros que precisam ser corrigidos, alguns fatores que não são levados em consideração em abordagens convencionais e que os métodos ágeis procuram incorporar. Este foi o motivo pelo qual o SCRUM foi escolhido para o desenvolvimento deste trabalho. 
...
\par O teste funcional é aquele onde a atividade de teste é voltada para as especificações, ou seja, com o funcionamento desejado de um programa, sem se preocupar na maneira como ele foi escrito. O critério adotado para a realização deste teste foi o conhecido como Análise do Valor Limite. Neste tipo teste, o domínio de entrada (ou seja, o conjunto de todos os possíveis valores que podem ser utilizados como entrada do programa~\cite{Delamaro}) é dividido em classes, tais que se espera um mesmo comportamento do programa para todos os elementos de uma mesma classe (por isso, estas são chamadas classes de equivalência)~\cite{Delamaro}. O critério se baseia em testar valores que estão nos limites de cada classe de equivalência, ou seja, na transição dos valores de entrada onde o comportamento do programa deve mudar.
