...
\par Em comparação com outros aplicativos, como apresentado na seção~\ref{sec:comparacao}, o simulador desenvolvido apresenta bons resultados, fazendo deste uma alternativa viável, principalmente por oferecer alguns recursos que não estão presentes em outros simuladores (como ferramentas de depuração). 
...
\par O mapa de memória também é uma funcionalidade que pode ser revista em termos de separação de informações, ou seja, mostrando diferentes abas com informações a respeito da pilha, registradores e RAM externa, por exemplo, facilitaria a leitura das diferentes regiões de memória e seu conteúdo. Pensando nisso também, o valor do PC (que não está mapeado em memória) também pode ser uma informação relevante para o usuário. Se utilizado em conjunto com um recurso de execução passo a passo do código, pode fazer do simulador uma ferramenta bastante útil para aqueles usuários mais interessados no estudo do processador em um nível mais baixo.

\par Outra funcionalidade interessante a ser implementada é a geração de arquivos de \textit{log}. Este recurso poderia ser integrado, por exemplo, no frequencímetro, dando a possibilidade de gravação da forma de onda que está sendo gerada para que o usuário possa visualizá-la em algum programa externo (como o \textit{gnuplot}), ou ainda, pensar na criação de um osciloscópio virtual. Esta é uma outra funcionalidade que traria o simulador para um nível mais acadêmico.
