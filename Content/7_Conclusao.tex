...
\par Como apresentado na seção~\ref{sec:comparacao}, o simulador desenvolvido apresenta resultados razoáveis em relação aos demais, não se destacando em nenhum ponto, mas mantendo-se dentro da média em todos os quesitos comparados, o que pode fazer deste uma alternativa viável, principalmente por oferecer alguns recursos que não estão presentes em outros simuladores (como ferramentas de depuração). A exceção fica apenas na medida de velocidade de simulação.

\par A velocidade de simulação que o sistema apresenta é certamente sua maior limitação, principalmente para aplicações envolvendo temporização. Para tempos da ordem de 10 ms,  ainda é possível de realizar simulações em um tempo razoável, no entanto escalas maiores farão com que a simulação demore a apresentar algum resultado. O principal problema neste caso fica na experiência de integração, onde o projeto passa pelo simulador para ser testado no Arduino posteriormente, forçando o usuário a fazer alterações no código em cada etapa.

\par Por fim, conclui-se que este projeto teve grande importância para o aprendizado de novas tecnologias, principalmente no que diz respeito ao desenvolvimento Android, programação para Arduino, organização e arquitetura do microcontrolador ATmega328P e teste de \textit{software}.
